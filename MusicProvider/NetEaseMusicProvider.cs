...
internal class NetEaseMusicProvider : IMusicProvider
{
    ...
    public async Task<List<string>> GetSearchSuggestAsync(string keyword)
    {
        try
        {
            ...
            {
                _logger?.LogError("解析网易搜索建议失败。", ex);
        ...
        {
            _logger?.LogError("网易搜索建议获取失败。", ex);
    ...
    public async Task<List<Music>> SearchAsync(string keyword)
    {
        ...
        try
        {
            ...
            {
                _logger?.LogError("解析网易搜索结果失败。", ex);
                return musics;
            }
            ...
            foreach (var song in result.result.songs)
            {
                ...
                {
                    _logger?.LogError("构建网易搜索结果失败。", ex);
                }
            }
        }
        catch (Exception ex)
        {
            _logger?.LogError("网易搜索失败。", ex);
        }
        return musics;
    }
    ...
    public async Task<List<SongMenu>> GetSongMenusFromTagAsync(string id, int page)
    {
        ...
        {
            _logger?.LogError("网易标签获取歌单失败。", ex);
    ...
    public async Task<string> GetPlayUrlAsync(string id, string extendDataJson = "")
    {
        ...
        {
            _logger?.LogError("网易获取播放地址失败。", ex);
            return "";
        }
    }
